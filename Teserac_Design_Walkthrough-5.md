...
- [Part 3 — From Raw Telemetry to Agent Context: The Compression Pipeline](#part-3--from-raw-telemetry-to-agent-context-the-compression-pipeline)
  ...
  - [3.5a SLA Breach Detection and Capacity Snapshot Population](#35a--sla-breach-detection-and-capacity-snapshot-population)
...
### 3.5a — SLA Breach Detection and Capacity Snapshot Population

> **TL;DR:** The `sla_breaches[]` section is populated by a rule-based comparison of live p99 latency against per-tenant SLA contracts stored in the Policy Store — not by Z-score scoring. The `capacity_snapshot[]` is a direct read from the Global Capacity DB, which is written exclusively by Temporal saga Activities and maintained independently of the telemetry pipeline.

#### How SLA Breaches Are Detected

SLA breach detection runs as a dedicated path within the cluster-level scoring service — parallel to, not downstream of, the Z-score anomaly scorer. It operates on the same normalized metric snapshots produced by the OTel Gateway, but applies a different evaluation: for each tenant whose `workload_type` is `inference` or whose `sla_tier` is `gold` or higher, it reads the tenant's p99 latency metric from the normalized snapshot and compares it against the per-tenant `sla_threshold_ms` value retrieved from the Policy Store.

The Policy Store is a separate, strongly consistent key-value store (backed by etcd or a Postgres table) that holds the SLA contract terms for each tenant — threshold values, tier definitions, and breach duration tolerances. It is not part of the telemetry pipeline; it is managed by the platform team and updated when tenant contracts are negotiated or modified. The SLA breach detector reads from it once per digest cycle, keyed by `tenant_id`.

A breach is declared when `current_p99_ms > sla_threshold_ms` for a sustained duration exceeding the tenant's breach tolerance window (typically 60–120 seconds). Transient latency spikes shorter than this window are not recorded as SLA breaches — they may appear as Z-score anomalies in the incident list, but they do not trigger the contractual breach path. Once declared, the breach record includes the tenant ID, the specific SLA metric, the threshold, the current observed value, the breach duration in seconds, and the affected cluster. This record is published to the `telemetry.incidents` Kafka topic alongside causal attribution incident records, so the Digest Assembler consumes both from a single source.

The SLA breach path bypasses the N=3 hysteresis gate that governs Z-score anomaly promotion — a single observation of `current_p99_ms > sla_threshold_ms` beyond the tolerance window is sufficient to declare a breach. This asymmetry is intentional: hysteresis exists to filter transient noise from utilization metrics; SLA breaches represent contractual violations where the tolerance window already serves the same debouncing function.

#### How the Capacity Snapshot Is Maintained

The `capacity_snapshot[]` section of the digest is not derived from telemetry scoring at all. It is a direct read from the Global Capacity DB — the strongly consistent key-value store (etcd or CockroachDB) that is the authoritative ledger for GPU allocation across the entire fleet. The Digest Assembler reads the Capacity DB at the start of each assembly cycle and incorporates a consistent point-in-time snapshot.

The Capacity DB is written exclusively by the `upsert_capacity_db` Temporal Activity, which runs as a step inside every saga that changes allocation state — `adjust_tenant_quota`, `move_tenant_workload`, `cross_cluster_migrate`, and any other action that transfers or modifies GPU allocations. Writes use compare-and-swap (CAS) for optimistic concurrency, ensuring that concurrent saga Activities do not silently overwrite each other's changes. No other service writes to the Capacity DB directly — the saga Activity is the single write path.

The Capacity DB also receives reads from the Kubernetes scheduler indirectly: when a pod is successfully scheduled onto a node, the `patch_resource_quota` Activity reads back the effective ResourceQuota from the Kubernetes API and verifies it matches the DB record via the `verify_quota_effective` Activity. This cross-verification is the mechanism that detects ghost state — divergence between what Kubernetes believes the quota to be and what the Capacity DB records.

The `capacity_horizon_hours` field in each cluster's snapshot deserves specific explanation. It is not read from any external system — it is computed by the Digest Assembler itself at assembly time, using the formula:

```
capacity_horizon_hours = available_gpus / max(gpu_demand_growth_rate_per_hour, ε)
```

where `gpu_demand_growth_rate_per_hour` is a rolling rate-of-change computed from the last N Capacity DB snapshots (typically N=6, covering the last 6 minutes), and `ε` is a small epsilon to avoid division by zero when demand is flat. A `capacity_horizon_hours` of 0.4 means the cluster will be fully exhausted in 24 minutes at the current demand growth rate — a strong signal for the agent to act preemptively rather than waiting for an active incident.

```
WHAT FEEDS EACH DIGEST SECTION:

active_incidents[]  ◄── Stage 2 Z-score scoring
                        Stage 3 incident clustering + causal attribution
                        (both running per-cluster, consuming telemetry.normalized)

sla_breaches[]      ◄── SLA breach detector (parallel to Z-score, per-cluster)
                        compares p99 latency vs. Policy Store thresholds
                        tolerance window replaces N=3 hysteresis
                        published to telemetry.incidents (same Kafka topic as incidents)

capacity_snapshot[] ◄── Global Capacity DB (direct read — not telemetry-derived)
                        written exclusively by upsert_capacity_db Temporal Activity
                        capacity_horizon_hours computed by Assembler from rolling rate-of-change

recent_actions[]    ◄── Action Log (written by saga execution layer on each SAGA_COMPLETED event)
                        covers last 30 minutes; read directly by Assembler

Policy Store        ──► feeds SLA thresholds into sla_breach detector
                        feeds SLA tier into capacity_snapshot.tenant_breakdown
```

...
*Three physical layers — node DaemonSet, cluster gateway, fleet assembler — each processes only what its scope allows.*

- **Node (DaemonSet):** scrapes DCGM + cAdvisor locally; applies 60s tumbling window + attribute reduction (drop pod UIDs) before data leaves the node
- **kube-state-metrics lives at the cluster gateway** — not node level; it watches the K8s API server for cluster-scoped objects (quotas, replicas) that nodes cannot see
- **Cluster Gateway:** joins node streams + kube-state-metrics + App OTLP spans → normalized (cluster, node, tenant, workload) snapshots → Kafka `telemetry.normalized`
- **Anomaly Scorer + Causal Attribution run per-cluster** — stateful, consuming from `telemetry.normalized`; output → Kafka `telemetry.incidents`
- **Digest Assembler (fleet level):** reads only `telemetry.incidents` — never raw metrics; also reads Global Capacity DB + SLA store + action log
- Result: millions of raw data points/sec → handful of named incidents → token-budgeted digest

**[TABLE: Telemetry Aggregation Hierarchy — Node / Cluster / Fleet]**
```
Layer      Component(s)                        Data In                        Data Out → Destination
──────────────────────────────────────────────────────────────────────────────────────────────────────
Node       OTel DaemonSet, DCGM Exporter,      Raw GPU metrics, container     60s windowed snapshots
           cAdvisor, node-problem-detector,     util, hw faults, kernel logs   (attribute-reduced)
           filelog receiver                                                    → Cluster Gateway

Cluster    OTel Standalone Gateway +            Node streams +                 Normalized (cluster,
           kube-state-metrics +                 cluster object state +         node,tenant,workload)
           App OTLP spans                       inference latency traces        → telemetry.normalized

Cluster    Anomaly Scorer (Z-score,             telemetry.normalized           Named incidents +
           per-cluster consumer) +              (per-cluster baseline)         root_cause_hypothesis
           Causal Attribution Engine                                           → telemetry.incidents

Fleet      Digest Assembler +                   telemetry.incidents +          Token-budgeted
           Global Capacity DB +                 fleet capacity + SLA store +   Cluster State Digest
           SLA Store + Action Log               recent saga outcomes           → AI Planning Agent

Fleet      AI Planning Agent (Loop 2) +         Digest + Capability Manifest   Plan → Plan Executor
           Capability Manifest Generator
```

**[DIAGRAM: Physical Topology — Node → Cluster → Fleet]**
```
NODE (DaemonSet — one per server)
  OTel Collector: DCGM + cAdvisor + node-problem-detector + filelog
  → 60s window + attribute reduction
  → sends to Cluster Gateway
          │
          ▼
CLUSTER (one gateway + scoring services per cluster)
  OTel Gateway: joins node streams + kube-state-metrics + App OTLP spans
  → normalized (cluster,node,tenant,workload) snapshots
  → Kafka: telemetry.normalized
  Anomaly Scorer → Causal Attribution Engine
  → Kafka: telemetry.incidents
          │
          ▼
FLEET (one — entire data center)
  Digest Assembler: reads telemetry.incidents + Global Capacity DB + SLA Store + Action Log
  → Cluster State Digest (token-budgeted JSON)
  AI Planning Agent: reads Digest + Capability Manifest → Plan
```
...
**[SUBSECTION: §3.5a — Where SLA and Capacity Data Come From]**

- **`sla_breaches[]`** — NOT from Z-score; separate rule-based path comparing live p99 latency against per-tenant SLA thresholds in the Policy Store
- SLA breach declared when `current_p99_ms > sla_threshold_ms` sustained beyond tolerance window (60–120 s) — tolerance window replaces N=3 hysteresis
- Policy Store: strongly consistent KV store (etcd/Postgres) holding SLA contract terms per tenant; managed by platform team, not telemetry pipeline
- **`capacity_snapshot[]`** — direct read from Global Capacity DB; NOT telemetry-derived
- Capacity DB written exclusively by `upsert_capacity_db` Temporal Activity (CAS writes) — no other service writes to it directly
- `capacity_horizon_hours` computed by Assembler: `available_gpus / gpu_demand_growth_rate_per_hour` (rolling rate from last 6 DB snapshots) — 0.4 hours = cluster exhausted in 24 min at current growth rate
- `recent_actions[]` — read directly from Action Log; written by saga execution layer on each SAGA_COMPLETED event

**[DIAGRAM: What Feeds Each Digest Section]**
```
active_incidents[]  ◄── Stage 2 Z-score + Stage 3 causal attribution (per-cluster, Kafka)
sla_breaches[]      ◄── SLA breach detector (parallel path) + Policy Store thresholds
                        tolerance window = debounce (not N=3 hysteresis)
capacity_snapshot[] ◄── Global Capacity DB (Temporal Activity writes only)
                        capacity_horizon_hours computed by Assembler from rolling rate-of-change
recent_actions[]    ◄── Action Log (saga SAGA_COMPLETED events, last 30 min)
```
